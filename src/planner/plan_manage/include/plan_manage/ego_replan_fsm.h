...
#include <traj_utils/GoalReached.h>
...
private:
  ...
  std::deque<traj_utils::MINCOTrajConstPtr> alltrajs_;
  ...
  ros::Publisher central_state_pub_, cen_replan_pub_, comm_delay_pub_;
  ...
  void CommDelayBroadcastMINCOTrajCallback(const traj_utils::MINCOTrajConstPtr &msg);
  void RecvBroadcastMINCOTrajCallback(const ros::TimerEvent &e);
  ...
  void groupCheck(std::vector<int> &openSet, std::vector<int> &closeSet, std::vector<Eigen::Vector3d> &other_odoms,
