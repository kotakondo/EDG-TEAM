...
void EGOReplanFSM::init(ros::NodeHandle &nh, ros::NodeHandle &nh1)
{
  ...
  nh_.param("fsm/group_dist", group_dist_, 4.0);  // group_dist_ - safe distance
  nh_.param("fsm/min_num", min_num_, 3);
  nh_.param("fsm/max_num", max_num_, 8);
  ...
  /* record plan state */
  // plan_state_pub_ = nh_.advertise<std_msgs::Int8>("planning/plan_state", 0);
  ...
  broadcast_ploytraj_pub_ = nh_.advertise<traj_utils::MINCOTraj>("planning/broadcast_traj_send", 10);
  broadcast_ploytraj_sub_ = nh_.subscribe<traj_utils::MINCOTraj>("planning/broadcast_traj_recv", 100,
                                                                 &EGOReplanFSM::CommDelayBroadcastMINCOTrajCallback,
                                                                 this, ros::TransportHints().tcpNoDelay());

  /* Use share local information(start, local target, and local map) in group planning */
  central_state_pub_ = nh_.advertise<traj_utils::CentralState>("/local_target_from_planner", 10);
  central_state_sub_ = nh_.subscribe("/local_target_to_planner", 10, &EGOReplanFSM::RecvBroadcastCentralStateCallback,
                                     this, ros::TransportHints().tcpNoDelay());

  /* Call replan in group planning */
  cen_replan_pub_ = nh_.advertise<std_msgs::UInt8MultiArray>("/cen_replan_from_planner", 10);
  cen_replan_sub_ = nh_.subscribe("/cen_replan_to_planner", 10, &EGOReplanFSM::RecvBroadcastCenReplanCallback, this,
                                  ros::TransportHints().tcpNoDelay());

  other_odoms_sub_ =
      nh_.subscribe("/all_odoms", 5, &EGOReplanFSM::otherOdomsCallback, this, ros::TransportHints().tcpNoDelay());

  comm_delay_pub_ = nh_.advertise<traj_utils::CommDelay>("comm_delay", 100);
  reach_goal_pub_ = nh_.advertise<std_msgs::Int32>("reach_goal_flag", 10);

  poly_traj_pub_ = nh_.advertise<traj_utils::PolyTraj>("planning/trajectory", 10);
  data_disp_pub_ = nh_.advertise<traj_utils::DataDisp>("planning/data_display", 100);
  heartbeat_pub_ = nh_.advertise<std_msgs::Empty>("planning/heartbeat", 10);

  if (target_type_ == TARGET_TYPE::MANUAL_TARGET)
  {
    waypoint_sub_ = nh_.subscribe("/goal", 1, &EGOReplanFSM::waypointCallback, this);
  }
    ...
    case CEN_EXEC_TRAJ: {
      ...
      if (have_recv_central_traj_ && flag_central_plan_)
      {
        ...
        if (t_cur >= 0)
        {
          ...
          else if (touch_the_goal && ((pos - wps_[waypoint_num_ - 1]).norm() < 1.5))  // local target close to the
                                                                                      // global target
          {
            if (!cen_reach_goal_)
            {
...
void EGOReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call)
{
  ...
  static string state_str[9] = { "INIT",          "WAIT_TARGET",    "GEN_NEW_TRAJ",
                                 "REPLAN_TRAJ",   "EXEC_TRAJ",      "CEN_REPLAN_TRAJ",
                                 "CEN_EXEC_TRAJ", "EMERGENCY_STOP", "SEQUENTIAL_START" };
...
void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent &e)
{
  ...
  size_t j_start = 0;
  // cout << "i_start=" << i_start << " pts_chk.size()=" << pts_chk.size() << " pts_chk[i_start].size()=" <<
  // pts_chk[i_start].size() << endl;
...
find_ij_start:;
  ...
  //   {
  //     cout << pts_chk[i][j].first << " @ " << pts_chk[i][j].second.transpose() << " @ " << (pts_chk[i][j].second -
  //     last_pt).transpose() << " @ " << map->getInflateOccupancy(pts_chk[i][j].second) << endl; last_pt =
  //     pts_chk[i][j].second;
  //   }
  // }
  ...
  for (size_t i = i_start; i < i_end; ++i)
  {
    for (size_t j = j_start; j < pts_chk[i].size(); ++j)
    {
      ...
      //   cout << "p=" << p.transpose() << " p_last=" << p_last.transpose() << " dist=" << (p -
      //   p_last).cwiseAbs().maxCoeff() << endl;
      // }
      // p_last = p;
...
bool EGOReplanFSM::callEcbsReplan()
{
  ...
  bool plan_success = planner_manager_->ecbsReplan(ecbs_start_pts_, ecbs_start_vels_, ecbs_start_accs_, ecbs_end_pts_,
                                                   ecbs_end_vels_, groupSet_, central_start_time_);
...
bool EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
{
  ...
  success = planner_manager_->planGlobalTrajWaypoints(odom_pos_, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(),
                                                      one_pt_wps, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero());
...
void EGOReplanFSM::CommDelayBroadcastMINCOTrajCallback(const traj_utils::MINCOTrajConstPtr &msg)
{
...
void EGOReplanFSM::RecvBroadcastMINCOTrajCallback(const ros::TimerEvent &e)
{
  // commdelay
  traj_utils::MINCOTrajConstPtr msg = alltrajs_[0];
  alltrajs_.pop_front();
  alltrajsTimers_.pop_front();
  if (!de_reach_goal_)
  {
    traj_utils::CommDelay comm_msg;
    comm_msg.comm_delay = (ros::Time::now() - msg->start_time).toSec();
  ...
  // we introduce comm delay, so comment out below
  // ros::Time t_now = ros::Time::now();
  // if (abs((t_now - msg->start_time).toSec()) > 0.25)
  // {
  //   if (abs((t_now - msg->start_time).toSec()) < 10.0)  // 10 seconds offset, more likely to be caused by unsynced
  //                                                       // system time.
  //   {
      ...
      for (int y = local_min(1); y <= local_max(1); ++y)
        for (int z = local_min(2); z <= local_max(2); ++z)
        {
          ...
          if ((msg->local_buffer[local_idx / 8] & (1 << (local_idx % 8))) == (1 << (local_idx % 8)))
            planner_manager_->grid_map_->md_
                .occupancy_buffer_inflate_[planner_manager_->grid_map_->toAddress(x, y, z)] = 1;
        }
  }
...
void EGOReplanFSM::groupCheck(std::vector<int> &openSet, std::vector<int> &closeSet,
                              std::vector<Eigen::Vector3d> &other_odoms, std::vector<int> &reach_goal)
{
